...
def load_checkpoint(config):
    try:
        ...
        checkpoint = max([x.split("-")[-1] for x in os.listdir(SESSION_DIR) if x.startswith("neat-checkpoint-")])
        print ("Found checkpoint at gen :" + str(checkpoint) + "... Loading...")
        return neat.Checkpointer.restore_checkpoint(SESSION_DIR + "neat-checkpoint-" + checkpoint)
    except ValueError:
...
def run(config, num_cores):
    ...
    pop.add_reporter(neat.Checkpointer(1, 600, SESSION_DIR + "neat-checkpoint-"))
